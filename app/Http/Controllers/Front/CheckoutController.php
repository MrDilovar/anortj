    ...
    public function cashondelivery(Request $request)
    {
        ...
        foreach($oldCart->items as $prod){    
            ...
            if($prod['item']['user_id']!=0  ){
                ...
                $sms_send = SmppAnor::sendSMS($vendor_data[$i]['vendor_phone'],"С вашего магазина оформлен заказ. Номер заказа:$order->order_number . Метод оплаты: При доставьке"); 
            }    
            $i++; 
        }
        
        $sms_send = SmppAnor::sendSMS("550000874"," Оформлен заказ: $order->order_number . Метод оплаты: При доставьке"); 
