      ...
      $prods = Product::when($cat, function ($query, $cat) {
                                  ...
                                  ->when($maxprice, function($query, $maxprice) {
                                   ...
                                   ->when($sort, function ($query, $sort) {
                                      ...
                                      elseif($sort=='popular_desc') {
                                        return $query->orderBy('views', 'DESC');
                                      }
                                      elseif($sort=='discount_desc') {
                                        return $query->where('is_discount','!=','NULL')->orderBy('id', 'DESC');
                                      }
                                      elseif($sort=='review_desc') {
              
                                         return $query->join('ratings', 'products.id', '=','ratings.product_id')->where('review','!=','NULL')->orderBy('review', 'DESC');   
                                      
                                      }
                                      elseif($sort=='rating_desc') {
                                        
                                        return $query->join('ratings', 'products.id', '=','ratings.product_id')->orderBy('rating', 'DESC');
                                      }
    ...
    // ------------------ Rating SECTION --------------------
        ...
        public function reviewsubmit(Request $request)
        {
            ...
            //   {
                  ...
                  if(count($prev) > 0)
                  {
                  return response()->json(array('errors' => [ 0 => 'Вы уже оценили этот товар' ]));
                  }
                  $Rating = new Rating;
                  $Rating->fill($request->all());
                  $Rating['review_date'] = date('Y-m-d H:i:s');
                  $Rating->save();
                  $data[0] = 'Ваш рейтинг успешно отправлен.';
