...
use App\Classes\KortiMilliSend;
use App\Classes\KortiMilliCheck;
...
class KortiMilliController extends Controller
{
    ...
    if($settings->is_kmilli == 1){
        ...
        $api_send = new KortiMilliSend($settings->kmilli_key, $settings->kmilli_user_name, route('kmilli.notify','id='.$item_number) );
    ...
    try {
            ...
            {   
             ...
             //Reponse From Korti Milli Api "
            $response =  $api_send->send($item_amount, $item_number, $part);
        
            $redirect_url = $response; /// redirect URL  to  Korti Milli payment page
    ...
    public function paykmilliCallback( Request $request ) {
            ...
            foreach($oldCart->items as $prod){    
                ...
                if($prod['item']['user_id']!=0  ){
                   ...
                   // $vendor_data[$i]['vendor_phone'] = '550000874';
                    $sms_send = SmppAnor::sendSMS($vendor_data[$i]['vendor_phone'],"С вашего магазина оформлен заказ. Номер заказа:$request->id . Метод оплаты: Корти Милли"); 
                }  
                $i++; 
            }

            $sms_send = SmppAnor::sendSMS('550000874',"Оформлен заказ: $request->id  . Метод оплаты: Корти Милли"); 
