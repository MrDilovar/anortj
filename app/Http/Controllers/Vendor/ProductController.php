...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->addColumn('status', function(Product $data) {
                                ...
                                // }
                                $class = $data->status == 1 ? 'drop-success' : ($data->status == 3 ? 'bg-info': 'drop-danger');
                                $s = $data->status == 1 ? 'selected' : '';
                                $ns = $data->status == 0 ? 'selected' : '';
                                ...
                                <option disabled data-val="1" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.$this->vendor_language->lang713.'</option>
                                <option data-val="3" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 3]).'" '.$ch.'>Changed</option>
                                <option disabled data-val="2" value="'. route('admin-prod-status',['id1' => $data->id, 'id2' => 2]).'" '.$w.'>Waiting</option>
                                <option disabled data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.$this->vendor_language->lang714.'</option></select></div>';
                            })
    ...
    public function status($id1,$id2)
    {
        ...
        $data->status = $id2;
        $data->changed_by = Auth::user()->email;
        $data->update();
    }
            ...
            if($i != 1)
            {
                ...
                if (!Product::where('sku',$line[0])->exists()){
                                ...
                                $prod->update();
                                    ...
                                    $err = 2;
                                    $log .= "<br>Row No: ".$i." - $line[1] category not found!<br>";
                                }

                }else{
                    $err = 2;
                    $log .= "<br>Row No: ".$i." - Duplicate Product Code: $line[0] <br>";
                }
        ...
        if (!empty($request->meta_tag))
         {
           ...
           if (empty($attrArr)) {
            ...
            // Default product status is deactivated
            $data['status'] = 0; 
            //Set product created user email 
    ...
    public function update(Request $request, $id)
    {
        ...
        if (empty($request->tags))
         ...
         $input['changed_by'] =  Auth::user()->email;
         $input['status'] = 3;
         $data->update($input);
        //-- Logic Section Ends
