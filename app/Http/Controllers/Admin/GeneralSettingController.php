...
class GeneralSettingController extends Controller
{
    ...
    public function kmilli($status)
    {

        $data = Generalsetting::findOrFail(1);
        $data->is_kmilli = $status;
        $data->update();
    }

    public function payler($status)
    {

        $data = Generalsetting::findOrFail(1);
        $data->is_payler = $status;
        $data->update();
    }

    public function installments($status)
    {

        $data = Generalsetting::findOrFail(1);
        $data->is_installments = $status;
        $data->update();
    }
