...
class OrderController extends Controller
{
    ...
    public function order_cancel($id)
    {
        ...
        if ($order->status == 'pending' || $order->status == 'processing' ){
            ...
            $order->reason_cancel=request()->reasonCancel;
            $order->update();
            return json_encode(['success'=>'Order successfully canceled']);
        }
        return json_encode(['errors'=>'продукты с такими статусом не отменяется']);
    } 
