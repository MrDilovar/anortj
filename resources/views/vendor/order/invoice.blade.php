                ...
                <div class="col-lg-6">
                    <div class="invoice__orderDetails">
                        ...
                        @if($order->method=="Installments")
                            <span> <strong>{{ __('Способ Оплаты') }} :</strong> {{__('В рассрочку')}}</span><br>
                            <span> <strong>{{ __('Предоплата (10%)') }} :</strong> {{$order->installments_prepay}}{{$order->currency_sign}}</span><br>
                            <span> <strong>{{ __('Срок ') }}{{$order->installments_for}} мес. :</strong> {{$order->installments_month_pay}}{{$order->currency_sign}}</span><br>
                        @else
                            <span> <strong>{{ __('Способ Оплаты') }} :</strong> {{$order->method}}</span>
                        @endif 
                    </div>
                </div>
            </div>
