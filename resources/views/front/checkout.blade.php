									...
									<div class="content">
										<div class="personal-info">
											...
											{{-- <div class="row">
												...
												<div class="col-lg-6">
													<input type="text" id="personal-phone" maxlength="9" class="form-control" name="personal_phone" placeholder="{{ __('Номер телефона') }}" value="{{ Auth::check() ? Auth::user()->phone : '' }}"  {!! Auth::check() ? 'readonly' : '' !!}> --}}
											...
											<div class="row set-account-pass d-none">
												...
												<div class="col-lg-6">
													<input type="text" id="personal-phone" maxlength="9" class="form-control" name="personal_phone" placeholder="{{ __('Номер телефона') }}" value="{{ Auth::check() ? Auth::user()->phone : '' }}"  {!! Auth::check() ? 'readonly' : '' !!}><br>
											...
											<div class="row">
												...
												<div class="col-lg-12">
													<input class="form-control" type="text" name="address"
														...
														value="{{ Auth::guard('web')->check() ? Auth::guard('web')->user()->address : '' }}" required>
												</div>
													<div class="col-lg-6 d-none">
														<select class="form-control" name="customer_country" required="">
															{{-- @include('includes.countries') --}}
															<option value="{{__('Tajiskistan') }}">{{__('Tajiskistan') }}</option>
														</select>
													</div>
												<div class="col-lg-6 d-none">
												...
												<div class="col-lg-6">
													<label for="shipping_date">{{ __('Дата доставки') }}</label>
													<input class="form-control" type="date"  name="shipping_date" id="shipping_date"
												min="{{date('Y-m-d')}}" max="{{date('Y-m-t')}}" value="" onchange="dateClick()" required >
												
												</div>
												<div class="col-lg-6">
													<label for="shipping_time">{{ __('Время доставки') }}</label>
													<select class="form-control" name="shipping_time" required>
														<option selected value="during">В течении дня</option>
															<option id="ten" value="10-12">10-12</option>
															<option id="twelve" value="12-14" >12-14</option>
															<option id="two" value="14-16" >14-16</option>
															<option id="four"  value="16-18" >16-18</option>	
													  </select>
												</div>
											</div>
										</div>
											...
											<div class="row">
												...
												<div class="col-lg-6">
													<input class="form-control ship_input" type="text" name="shipping_phone"
														id="shipingPhone_number" maxlength="9" placeholder="{{ $langg->lang153 }}">
												</div>
											</div>
											...
											<div class="row">
												<div class="col-lg-6">
													<input class="form-control ship_input" type="text" name="shipping_city"
														id="shipping_city" placeholder="{{ $langg->lang158 }}" value="Душанбе" readonly>
												</div>
												{{-- <div class="col-lg-6">
											...
											<div class="payment-information">
												...
												<div class="row">
													<div class="col-lg-12">
														...
														@endif
														<!-- Korti Milli  payment link section start -->						
														@if($gs->is_kmilli == 1)
															<a class="nav-link payment" data-val="" data-show="no" data-form="{{route('kmilli.submit')}}" data-href="{{ route('front.load.payment',['slug1' => 'milli','slug2' => 0])  }}" id="v-pills-tab1-tab" data-toggle="pill" href="#v-pills-tab1" role="tab" aria-controls="v-pills-tab1" aria-selected="false">
																	<div class="icon">
																			<span class="radio"></span>
																	</div>
																<p>
																	{{__('Korti Milli')}} <img  class="w-auto" src="{{ asset('assets/front/images/korti-milli-logo.png') }}" alt="Korti Milli">
																	@if($gs->kmilli_user_name != null)
														...
														@endif
														<!-- Korti Milli  payment link section end -->

														<!-- Payler Visa  payment link section start -->						
														@if($gs->is_payler == 1)
														<a class="nav-link payment" data-val="" data-show="no" data-form="{{route('payler.submit')}}" data-href="{{ route('front.load.payment',['slug1' => 'payler','slug2' => 0])  }}" id="v-pills-tab1-tab" data-toggle="pill" href="#v-pills-tab1" role="tab" aria-controls="v-pills-tab1" aria-selected="false">
																<div class="icon">
																		<span class="radio"></span>
																</div>
															<p>
																{{__('VISA  ')}} <img  class="w-auto" src="{{ asset('assets/front/images/visa-logo.png') }}" alt="Visa">
																<!-- @if($gs->kmilli_user_name != null)
													...
													@endif
													<!-- Payler Visa  payment link section end -->

														<!-- Installment  payment link section start -->
														@if(Session::has('cart'))
														   @if( App\Models\Product::convertPrice(Session::get('cart')->totalPrice) >=400  )
																					
															    @if($gs->is_installments == 1) 
																<a class="nav-link payment" data-val="" id="id_installments" data-show="no" data-form="{{route('installments.submit')}}" data-href="{{ route('front.load.payment',['slug1' => 'installments','slug2' => 0])  }}" id="v-pills-tab1-tab" data-toggle="pill" href="#v-pills-tab1" role="tab" aria-controls="v-pills-tab1" aria-selected="false">
																		<div class="icon">
																				<span class="radio"></span>
																		</div>
																		<p>Рассрочка <p>
																</a>
																<div class="container w-75 p-3 ml-0 mb-2 rounded d-none " id="credit_sec" style="border:1px dashed rgba(0, 0, 0, 0.3); ">
																	<div class="row p-3 pt-0 ">
																		
																		<p class=" mr-2  " >В рассрочку на:</p>
																		{{-- <p class="price"><span id="sizepriceCredit"></span></p> --}}
																		
																		<div class="btn-group btn-group-toggle  ml-3 " data-toggle="buttons">
																			<label class="btn btn-outline-primary  " >
																			<input type="radio"  name="optionsCredit" id="option3" value="3" > 3 мес.
																			</label>
																			<label class="btn btn-outline-primary">
																			<input type="radio" name="optionsCredit" id="option6" value="6"> 6 мес.
																			</label>
																			<label class="btn btn-outline-primary">
																			<input type="radio" name="optionsCredit" id="option12" value="12"> 12 мес.
																			</label>
																			<label class="btn btn-outline-primary">
																			<input type="radio" name="optionsCredit" id="option18" value="18"> 18 мес.
																			</label>
																		
																		</div>  
																	</div>
																	<p class=" pt-2 d-none" id="commission"></p>
																	<p class=" pt-2 d-none" id="prepayment">Предоплата :</p>
																	<p class=" pt-2 d-none" id="monthly_payment"></p>
																	<input type="hidden" name="credit_prepayment" id="credit_prepayment" value="">
																	<input type="hidden" name="credit_month_pay"  id="credit_month_pay" value="">
																</div>
															    @endif 
															<!-- Installment  payment link section end -->
															@endif
														@endif	
				...
				<div class="col-lg-4">
					<div class="right-area">
						...
						<ul class="order-list">
							...
							<P>
								<b
								class="cart-total" id="price_credit">{{ Session::has('cart') ? App\Models\Product::convertPrice(Session::get('cart')->totalPrice) : '0.00' }}</b>
							</P>
							</li>
						...
						<div class="packeging-area">
							...
							@foreach($shipping_data as $data)
								...
								<div class="radio-design">
										@if($totalPrice  < 50)
											<input type="radio" class="shipping" id="free-shepping{{ $data->id }}" name="shipping" value="{{$data->price}}" {{ ($loop->first) ? 'checked' : '' }}> 
											@else 
											<input type="radio" class="shipping" id="free-shepping{{ $data->id }}" name="shipping" value="0" {{ ($loop->first) ? 'checked' : '' }}> 
										@endif
										<span class="checkmark"></span>
										<label for="free-shepping{{ $data->id }}"> 
												{{-- {{ $data->title }} --}}
												{{ __('Стандартная') }}
												@if($data->price != 0 && $totalPrice  < 50 )
												+ {{$data->price}} {{ $curr->sign }}
																		
												@endif
												{{-- <small>{{ $data->subtitle }}</small> --}}
												<small>{{__('Заказы от 50 сомони бесплатно. ')}}</small>
												<small>{{__('Заказы до 50 сомони 5 сомони за доставку')}}</small>
...
function dateClick(){
	var curr_d = formatDate($.now());
	var givven_d = $('#shipping_date').val();
	var dt = new Date();
	if(curr_d == givven_d){
		if(dt.getHours() > 10){ $("#ten").attr('disabled',true) };
		if(dt.getHours() > 12){ $("#twelve").attr('disabled',true) };
		if(dt.getHours() > 14){ $("#two").attr('disabled',true) };
		if(dt.getHours() > 16){ $("#four").attr('disabled',true) };
	}else{
		$("#ten").attr('disabled',false);
		$("#twelve").attr('disabled',false);
...
var fixed_total =$('#grandtotal').val().replace(',','.');
var ftotal = parseFloat(fixed_total) + mship + mpack;
...
$(document).on("click",()=> {
   if($('#id_installments').attr('aria-selected')=='true') {
	  $("#credit_sec").removeClass('d-none');

   }
   else if($('#id_installments').attr('aria-selected')=='false'){
	  $("#credit_sec").addClass('d-none');	
   }	  
});

$("input[name=optionsCredit]").change(()=>{
//get value of of input which of them has checked prop
let credit_val = $( 'input[name=optionsCredit]:checked' ).val();
//get the value of product price
let price = parseInt($("#price_credit").text().replace('S',''));
//set valeu of hidden post field
$("#credit_prepayment").val((price * 0.1).toFixed(2));
$("#credit_month_pay").val(((price-(price*0.1))/credit_val).toFixed(2));
//show comission and prepayment section
$("#commission").removeClass("d-none");
$("#commission").html("Коммисия : 0%");
$("#prepayment").removeClass("d-none");
$("#prepayment").html("Предоплата : (10%) "+(price * 0.1).toFixed(2)+" S");
 //change the installment price 
// $("#sizepriceCredit").text(((price-(price*0.1))/credit_val).toFixed(1)+"S");
$("#monthly_payment").removeClass("d-none");
$("#monthly_payment").html("Ежемесячный платеж : "+((price-(price*0.1))/credit_val).toFixed(2)+
" S   <span class='ml-2'>Итого:</span> <span class='font-weight-bold text-danger'>"+$("#price_credit").text()+"</span>");
